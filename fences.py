...
counting_regions = [
    {
        "name": "fance 1",
        "polygon": Polygon(data["fences"][0]),  # Polygon points
        "counts": 0,
        "dragging": False,
    ...
    },
    {
        ...
        "polygon": Polygon(data["fences"][1]),  # Polygon points
        "counts": 0,
        "dragging": False,
        "region_color": (0, 255, 0),  # BGR Value
        "text_color": (0, 0, 0),  # Region Text Color
...
def run(
    ...
    video_writer = cv2.VideoWriter(str(save_dir / "output_fences.mp4"), fourcc, fps, (frame_width, frame_height))
        ...
        if results[0].boxes.id is not None:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                ...
                for region in counting_regions:
                    ...
                    if region["polygon"].contains(Point((bbox_center[0], bbox_center[1]))) and cls==0:
                        region["counts"] += 1
                        
                        cv2.polylines(frame, [polygon_coords], isClosed=True, color=(0, 0, 255), thickness=region_thickness)
                        cv2.rectangle(
                            frame,
                            (text_x - 5, text_y - text_size[1] - 5),
                            (text_x + text_size[0] + 5, text_y + 5),
                        ...
                        cv2.putText(frame, str(region["counts"]), (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, region_text_color, line_thickness)
